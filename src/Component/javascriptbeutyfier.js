...
const JavascriptBeutyfiers = () => {
    const [result, setResult] = useState('');

    const onSubmit = async () => {
        const data = await beautify_js(result);
        setResult(data);
    };
                    ...
                    <div className="main">
                        <article id="post-59" className="post-59 page type-page status-publish hentry">
                            <header className="entry-header">
                                <h1 className="entry-title" style={{color:"white"}}>JavaScript beautifier</h1>
                            </header>
                                        ...
                                        <div className="minify_area_left">
                                            <div className="form-item">
                                                <div className="form_item_left"><label
                                                    htmlFor="js_enter_code" style={{color:"white"}}>Enter <strong>JavaScript</strong> code then
                                                    click <strong>Beautify JavaScript</strong> button below to Beautify:</label></div>
                                                <div className="form_item_right">
                                                    <textarea cols={65} rows={8}
                                                              style={{width:"99%",background:"#2e3e4e"}}
                                                              onChange={(e) => setResult(e.target.value)}
                                                              name="js_enter_code" value={result} className="enter_code"
                                                              defaultValue={""}/>
                                                </div>
                                            </div>
                                            <div className="form-item">
                                                <input type="submit" value="Beautify JavaScript" onClick={onSubmit}
...
export default JavascriptBeutyfiers